...
def food_details(fdc_id):
    # Fetch the food data using the existing API key and base URL for a specific FDC ID
    ...
    if response.status_code == 200:
        ...
        detail = {
            ...
            'Data Type': food.get('dataType'),
            'Brand Owner': food.get('brandOwner', 'N/A')
            # 'Ingredients': food.get('ingredients', 'N/A')  # Uncomment this line if you need ingredients.
        }
        return detail
    else:
        print(f"Failed to fetch food data: {response.status_code}")
        return None
